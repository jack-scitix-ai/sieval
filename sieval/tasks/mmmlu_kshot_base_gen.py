"""MMMLU full multilingual k-shot base-model logprob task.
...
@sieval_task(
    ...
    display_name="MMMLU (k-shot, base logprob)",
    ...
    reference_impl=ReferenceImpl(
        ...
        notes=(
            "Uses MMMLU locale/category/subject organization and size-weighted "
            "aggregation from lm-evaluation-harness; uses Hendrycks evaluate.py "
            "completion top-logprob scoring for base models."
        ),
    ),
)
class MMMLUKShotBaseGenTask(
    Task[
        MMMLUDatasetSample,
        str,
    ...
    async def report(
        ...
        if total == 0:
            return {"score": 0.0, "score_mmmlu": 0.0, "fails": 0, "pass@1": 0.0}
        ...
        for ctx in fails:
            raw = ctx.raw_sample
            locale = self._locale(raw) if raw is not None else "unknown"
            category = self._category(raw) if raw is not None else "unknown"
            subject = self._subject(raw) if raw is not None else "unknown"
            self._add_group_metrics(
                metric_counts,
                locale=locale,
                category=category,
